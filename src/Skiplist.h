...
public:
	...
	Map(std::initializer_list<std::pair<const Key_T, Mapped_T>> valList) {
		maxLevel = 100;
		currLevel = 0;
		sizeVal = 0;
		head = new SLNode(maxLevel);
	...
	public:
		...
		SLNode(const std::pair<const Key_T, Mapped_T>* newVal, int level) {
			ValueType *retVal = new std::pair<const Key_T, Mapped_T>(*newVal);
			key = retVal;
	...
	Mapped_T &operator[](const Key_T &key) {
		ValueType test = std::pair<const Key_T, Mapped_T>(key, Mapped_T());
	...
	std::pair<Iterator, bool> insert(const ValueType &newVal) {
		SLNode *current = head;
		Key_T key = std::get<0>(newVal);
		std::vector<SLNode*> update(maxLevel + 1, nullptr);

		for (int i = currLevel; i >= 0; i--)
		{
			while (current->next[i]->key != nullptr && std::get<0>(*(current->next[i]->key)) < key) {
	...
	friend bool operator==(const Map &lhs, const Map &rhs) {
		if(lhs.size() == rhs.size()) {
			...
			auto itr = rhs.begin();
			while(itl != lhs.end()) {
				if(!(std::get<0>(*(itl)) == std::get<0>(*(itr))) || !(std::get<1>(*(itl)) == std::get<1>(*(itr)))) {
					return false;
				}
				++itl;
				++itr;
